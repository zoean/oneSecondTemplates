...
var albumTempEdit = {
    ...
    initSongId:selectSongId,//接口里默认音乐id
    initTemplateId:selectTemplateId,//接口里默认模板id
  ...
  playMusic:function (){
	...
	getTempData:function (){//获取模板列表并编辑模板
		...
		$.getJSON(this.tempUrl,function (json){
			...
			$("#tempWrap ul li").unbind('click').click(function(){				
				...
				// 选择模板预览
				$(this).attr("id");
				console.log('../edit-album/edit-album?id='+that.albumId + '&templateId=' + that.curTempId + '&tempMusicId=' +that.curMusicId  + '&viewType=0')
	...
	getMusicData:function (){
		...
		$.getJSON(this.musicUrl,function (json){
			...
			var musicLength = json.content.length;	
			var str = "";
			var lineCount = Math.ceil(musicList.length/2);//根据音乐条数计算出行数
			console.log(lineCount + '几行')
			...
			if(musicList && musicList.length>0){
				for(var i=0;i<musicList.length;i++){
					...
					if(musicList[i].id == that.curMusicId){
						str += '<li class="cur" id="'+musicList[i].id+'" music="'+musicList[i].songUrl+'"><h3>'+subString(musicName,14,true)+'</h3><p>'+subString(musicSinger,18,true)+'</p></li>';
					}else{
						str += '<li id="'+musicList[i].id+'" music="'+musicList[i].songUrl+'"><h3>'+subString(musicName,14,true)+'</h3><p>'+subString(musicSinger,18,true)+'</p></li>';
					}
				}
			}
			//音乐列表加载完成后初始化iscroll
